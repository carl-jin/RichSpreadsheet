...
import { setCellValue } from "../global/apiHelper";

//选区下拉
const luckysheetDropCell = (window.__luckysheetDropCell =
  window.__luckysheetDropCell
    ? window.__luckysheetDropCell
    : {
        copyRange: {}, //复制范围
        applyRange: {}, //应用范围
        ...
        isChnNumber: function (txt) {
          ...
          if (txt.length == 1) {
            ...
            for (let i = 0; i < str.length; i++) {
              if (
                !(str[i] in _this.chnNumChar || str[i] in _this.chnNameValue)
              ) {
                isChnNumber = false;
                break;
              }
  ...
  /!*            $("body").append(
         ...
         /!*     let typeItemHide = _this.typeItemHide();
              if (
                ...
                $(
                  "#luckysheet-dropCell-typeList .luckysheet-cols-menuitem[data-type=1]"
                ).hide();
                $(
                  "#luckysheet-dropCell-typeList .luckysheet-cols-menuitem[data-type=4]"
                ).hide();
                $(
                  "#luckysheet-dropCell-typeList .luckysheet-cols-menuitem[data-type=5]"
                ).hide();
                $(
                  "#luckysheet-dropCell-typeList .luckysheet-cols-menuitem[data-type=6]"
                ).hide();
                $(
                  "#luckysheet-dropCell-typeList .luckysheet-cols-menuitem[data-type=7]"
                ).hide();
                $(
                  "#luckysheet-dropCell-typeList .luckysheet-cols-menuitem[data-type=8]"
                ).hide();
              }
              ...
              $(
                "#luckysheet-dropCell-typeList .luckysheet-cols-menuitem[data-type=" +
                  type +
                  "]"
              )
                .find("span")
            ...
            if (applyD_date.length > 0) {
              for (let y = 0; y < applyD_date.length; y++) {
                if (x in applyD_date[y]["index"]) {
                  applyData.push(
                    applyD_date[y]["data"][applyD_date[y]["index"][x]]
                  );
                }
              }
            }
          ...
          if (type == "0") {
            ...
            if (dataType == "number") {
              ...
              if (data.length == 1) {
                if (
                  data[0]["m"] == "日" ||
                  _this.ChineseToNumber(data[0]["m"]) < 7
                ) {
                  //数字小于7，以周一~周日序列填充
                  let step;
                  if (direction == "down" || direction == "right") {
            ...
            //不带格式填充
            let dataArr = _this.getDataByType(
              data,
              len,
              direction,
              "1",
            ...
            if (data.length == 1) {
              ...
              if (
                ...
                //日不一样
                if (
                  Math.abs(dayjs(data[1]["m"]).diff(dayjs(data[0]["m"]))) > 7
                ) {
        ...
        FillChnWeek: function (data, len, step) {
          ...
          for (let i = 1; i <= len; i++) {
            ...
            if (data[data.length - 1]["m"] == "日") {
              ...
              num =
                _this.ChineseToNumber(data[data.length - 1]["m"]) + step * i;
            }
        ...
        judgeDate: function (data) {
          ...
          let equalDiffDays = dayjs(data[1]["m"]).diff(
            dayjs(data[0]["m"]),
            "days"
          );
          ...
          let equalDiffYears = dayjs(data[1]["m"]).diff(
            dayjs(data[0]["m"]),
            "years"
          );
...
export default window.__luckysheetDropCell;
