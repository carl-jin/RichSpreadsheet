...
const sheetmanage = {
  ...
  /**
   * 将 {id:columnId,field1:value,file2:value} 这样的一条数据转换成
   * cell data https://mengshukeji.github.io/LuckysheetDocs/zh/guide/cell.html#%E5%9F%BA%E6%9C%AC%E5%8D%95%E5%85%83%E6%A0%BC
   //  **** cell value 转化内
   * @param column
   * @param celldata
   */
  transformGridDataToCellData: function (column, celldata) {
    let cellDataArr = [];
    let colsIndex = {};

    function getColIndexById(colId) {
      if (colsIndex[colId] !== undefined) {
        return colsIndex[colId];
      }
      let col = column.find((_col) => _col.id === colId);
      let index = column.indexOf(col);
      colsIndex[colId] = index;
      return index;
    }
