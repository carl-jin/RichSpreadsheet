...
import { setCopyToClipboard } from "./hooks/useGsClipboard";
...
const selection = {
  ...
  copy: function (e) {
    ...
    //  **** copy
    setCopyToClipboard(rowIndexArr, colIndexArr);
    Store.iscopyself = true;
    return false;
    /*console.log(rowIndexArr, colIndexArr);
        ...
        for (let h = mth; h < maxrowCache; h++) {
          ...
          for (let c = mtc; c < maxcellCahe; c++) {
            ...
            if (value != null) {
              ...
              if (x[c].ct && x[c].ct.t === "inlineStr") {
                x[c].ct.s.forEach(
                  (item) => (item = $.extend(true, item, value))
                );
              }
