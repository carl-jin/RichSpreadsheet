...
const hyperlinkCtrl = {
  ...
  init: function () {
    ...
    $(document)
      ...
      .on(
        "change.linkType",
        "#luckysheet-insertLink-dialog-linkType",
        function (e) {
    ...
    $(document)
      ...
      .on(
        "click.confirm",
        "#luckysheet-insertLink-dialog-confirm",
        function (e) {
          let last =
  ...
  cellFocus: function (r, c) {
    ...
    if (item.linkType == "external") {
      ...
      if (sheetIndex != Store.currentSheetIndex) {
        ...
        $("#luckysheet-sheets-item" + sheetIndex).addClass(
          "luckysheet-sheets-item-active"
        );
  ...
  overshow: function (event) {
    ...
    let html = `<div id="luckysheet-hyperlink-overshow" style="background:#fff;padding:5px 10px;border:1px solid #000;box-shadow:2px 2px #999;position:absolute;left:${col_pre}px;top:${
      row + 5
    }px;z-index:100;">
                        <div>${linkTooltip}</div>
                        <div>单击鼠标可以追踪</div>
  ...
  ref: function (historyHyperlink, currentHyperlink, sheetIndex, d, range) {
    ...
    Store.luckysheetfile[getSheetIndex(sheetIndex)].hyperlink =
      currentHyperlink;
