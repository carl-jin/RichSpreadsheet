      ...
      if (cell != null && (cell.v != null || isInlineStringCell(cell))) {
        ...
        if (cell.mc) {
          if (c === cell.mc.c) {
            ...
            let ed_cellWidth = colLocationByIndex(
  ...
  for (let strI = 1; strI <= strValue.length; strI++) {
    ...
    if (strtextMetrics > cellWidth) {
      if (strI - 1 <= 0) {
        ...
        return getCellTextSplitArr(
          strValue.substring(strI - 1),
          strArr,
          cellWidth,
          canvas
        );
  ...
  if (mtc != null) {
    ...
    /*   if (
      ...
      let oneLineTextHeight =
        menuButton.getTextSize(commonWord, ctx.font)[1] * 0.8;
...
function getCellTextInfo(cell, ctx, option) {
  ...
  let tb = 0; //wrap overflow
  let tr = 0; //rotate
  let rt = 0; //rotate angle
  ...
  /*if (tr == "3") {
    ...
    /*if (tb == "2" || isInline) {
      ...
      let measureText = getMeasureText(value, ctx, fontset);
      let textWidth = measureText.width;
      let textHeight =
        measureText.actualBoundingBoxDescent +
        measureText.actualBoundingBoxAscent;
